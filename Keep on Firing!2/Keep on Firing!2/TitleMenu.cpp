...
void TitleMenu::update()
{
	...
	if (stopwatch1.isActive())
	{
		...
		if (Input::KeyEnter.clicked || Input::KeyZ.clicked)
		{
			if (texts[selectIndex] == L"‚Í‚¶‚ß‚é") {
				changeScene(L"Game");
			}
			if (texts[selectIndex] == L"‚¨‚í‚é") {
				System::Exit();
			}
		}
	}
...
void TitleMenu::draw() const
{
	...
	const double e0 = EaseOut(Easing::Quad, t0);

	if (e0 > 0.0)
	{
		{
			const Vec2 center(Window::Center());
			Graphics2D::SetTransform(Mat3x2::Translate(-center).scale(e0).translate(center));
			RoundRect(messageBox, 20).draw(Color(20, 30, 120));
		}
		const int windowCenterX = Window::Center().x;

		{
			...
			RectF(180, 50).setCenter(windowCenterX, y).drawShadow({ 0,0 }, 10, 0, { 1.0,0.2*e0 });
		}
		...
		for (auto& i : step(texts.size()))
		{
			...
			const Vec2 center(windowCenterX, 200 + i * 50);
